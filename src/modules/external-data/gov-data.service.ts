...
export class GovDataService {
        ...
        try {
            const response = await axios.get(this.BASE_URL, {
                params: {
                    ...
                    filters: JSON.stringify({ "CityName": city }),
                    limit: 5
                }
            });
            ...
            return records.map((r: any) => ({
                name: r.StationTypeName || "תחנת אוטובוס",
                lines: "קווים עירוניים"
            }));
        } catch (error: any) {
        ...
        try {
            const response = await axios.get(this.BASE_URL, {
                params: {
                    ...
                    filters: JSON.stringify({ "SETL_NAME": city }),
                    limit: 5
                }
            });
    ...
    getEnrichedNeighborhoodData = async (lat: number, lng: number, city: string, address?: string) => {
        ...
        const [busStops, schools, urbanRenewal] = await Promise.all([
